...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSInteger selected = ((AppDelegate *)[UIApplication sharedApplication].delegate).selectedTo;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.searchDisplayController isActive]) {
        if ([language isEqualToString:@"fr"]) {
            parent.to.text = [[searchResults objectAtIndex:indexPath.row] objectForKey:@"labelFR"];
        }
        else {
            parent.to.text = [[searchResults objectAtIndex:indexPath.row] objectForKey:@"label"];
        }
        shortcode = [[searchResults objectAtIndex:indexPath.row] objectForKey:@"shortcode"];
        ...
        if ([language isEqualToString:@"fr"]) {
            parent.to.text = [[rates objectAtIndex:indexPath.row] objectForKey:@"labelFR"];
        }
        else {
            parent.to.text = [[rates objectAtIndex:indexPath.row] objectForKey:@"label"];
        }
        shortcode = [[rates objectAtIndex:indexPath.row] objectForKey:@"shortcode"];
        ((AppDelegate *)[UIApplication sharedApplication].delegate).selectedTo = indexPath.row;
